...
contract LiquidityMigratorV1 is Ownable, ReentrancyGuard {
    ...
    constructor(IFarm _farm, IRewardManager _rewardManger, IFYGN _fYGN) {
        farm = _farm;
    ...
    function switchLiquidity(
        ...
        require(
            _oldPid < farm.poolLength() && _newPid < farm.poolLength(),
            "Pool does not exist"
        );
        ...
        */
        ...
        //@audit-ok - This line will ensure that user has enough balance for the migration.
        farm.withdrawFor(_oldPid, _amount, msg.sender, false);
        uint256 afterWithdrawTokenBalance = _token.balanceOf(address(this));
        uint256 newDepositAmount = afterWithdrawTokenBalance.sub(
            beforeWithdrawTokenBalance
        );
        ...
        //@audit-issue - but this is contract's balance not users balance.
    ...
    // Rescue any "any" tokens that have not been able to processed by the contract
